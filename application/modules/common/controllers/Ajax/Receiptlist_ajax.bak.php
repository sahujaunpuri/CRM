			...
			foreach ($result2 as $key) {
				...
				if($amount_total > 0){
					if($key->sign == '+' ){
						...
						<td class="text-center"><button type="button" onclick="addInvoice(this)" id="invoiceAdd-'.$key->ar_id.'" class="btn btn-success" >Select</button><span> </span><button type="button" class="btn btn-danger display-none"  id="invoiceRemove-'.$key->ar_id.'" onclick="removeInvoice(this)">Unselect</button></td>
						</tr>';
					} else {
						$creditNotes .= 
						'<tr id="'.$key->ar_id.'">
						...
						<td class="text-center"><button type="button" onclick="addCredit(this)" id="creditAdd-'.$key->ar_id.'" class="btn btn-success">Select</button><span> </span><button type="button" class="btn btn-danger display-none" id="creditRemove-'.$key->ar_id.'" onclick="removeCredit(this)">Unselect</button></td>
						</tr>';
					}
				}
			}
