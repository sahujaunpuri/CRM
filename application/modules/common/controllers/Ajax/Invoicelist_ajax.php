...
defined('BASEPATH') OR exit('No direct script access allowed');
	class Invoicelist_ajax extends CI_Controller {
		...
		public function get_customer_details()
		{
			...
			$data['customer_address']=$result->customer_bldg_number.' , '.$result->customer_street_name.' , '.$result->customer_postal_code;
			//$data['customer_address']=$result->customer_street_name.' , '.$result->customer_postal_code;
    ...
    else
    {
		...
		public function print_new_invoice($mode="print")
		{
			...
			foreach ($billing_id_array as $key => $billing_id) {
                ...
                $detailed_description = $this->custom->getSingleValue('detail_description_invoice','description_invoice',array('invoice_ref_no' => $this->input->post('invoice_ref_no'),'billing_id' => $billing_id));
		...
		public function print_invoice($mode="print")
		{
			...
			if($invoice_edit_data):
				...
				//$data['customer_address']=$result->customer_street_name.' , <br>'.$result->customer_postal_code;
				$data['customer_phone']=$result->customer_phone;
				$data['customer_email']=$result->customer_email;
				$currency_data=$this->custom->getSingleRow("currency_master",array('currency_id'=>$result->currency_id));
				$data['customer_currency']=$currency_data->currency_name;
            ...
            if(!empty($to)) {
	            ...
	            $this->email->to($to);
	            $this->email->subject('information about invoice');
        ...
        public function add_detail_description(){
            $result = $this->invoice->addDescription();
            return $result;
        }

        public function get_detail_description(){
            $result = $this->invoice->getDescription();
            echo $result;
        }
