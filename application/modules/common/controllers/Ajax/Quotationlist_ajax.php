	...
	public function get_product_row()
		{
			...
			$html="";
			$html.="<td style='max-width: 600px; white-space: normal;'><span id='billing-id-$product_details->billing_id' >".$product_details->billing_description."</span><br><span id='detailsAdded-$product_details->billing_id' name='detailsAdded-$product_details->billing_id'></span></td>";
	        ...
	        if (isset($post['currencyRate']) && $post['currencyRate']!='0') 
            {
                ...
                if($product_details->billing_type=="Product")
                {
                    $html.="<td style='max-width: 100px;' id='unit_price_".$product_details->billing_id."'>".$currencyFinal."</td>";
                }
    
                else
                {
                    $html.="<td style='max-width: 100px;' id='unit_price_".$product_details->billing_id."'></td>";
                }
            }
...
//            $result = array('key' => $html, 'key2' =>$product_details->billing_id );
...
// 			echo $result;
		...
		public function print_new_quotation($mode="print")
		{
			...
			foreach ($billing_id_array as $key => $billing_id) {
                ...
                $detailed_description = $this->custom->getSingleValue('detail_description_quotation','description_quotation',array('quotation_ref_no' => $this->input->post('quotation_ref_no'),'billing_id' => $billing_id));
		...
		public function add_detail_description(){
            $result = $this->quotation->addDescription();
            return $result;
        }

        public function get_detail_description(){
            $result = $this->quotation->getDescription();
            echo $result;
        }
	}
