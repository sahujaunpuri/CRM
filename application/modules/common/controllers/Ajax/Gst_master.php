...
class Gst_master extends CI_Controller {
	...
	public function delete(){
		...
		$where = array('gst_id' => $id);
		//$result = $this->custom->deleteRow($this->table,$where);
		$code = $this->custom->getSingleValue($this->table,"gst_code",$where);
		$wherecode = array('currency_name' => $code);
		$i = 0;
		$flag = 0;
		do{
			if($i == 0){
				$i++;
				$checkBillingResult = $this->custom->checkTableValues('billing_master',$where);
