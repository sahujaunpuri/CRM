...
class Receiptlist_ajax extends CI_Controller {
	...
	{ 
		...
		if(isset($post['edit']) && $post['edit'] == 'edit'){
			...
			$sql = 'SELECT ar_id, doc_ref_no, total_amt,doc_date, sign FROM accounts_receivable WHERE customer_code = "'.$result->customer_code.'" AND settled = "n" AND tran_type != "Rec" ORDER BY doc_date ASC';
			$query = $this->db->query($sql);
			$result2 = $query->result();			
			foreach ($result2 as $key) {
				$sql = 'SELECT SUM(rec_inv_amount) AS sum_partial FROM receipt_invoice_master WHERE partial_status = "P" AND invoice_id = '.$key->ar_id.' ';
				$partial_payments = $this->db->query($sql);
				$partial_for_this = $partial_payments->result();
				$partial_for_this_number = number_format($partial_for_this[0]->sum_partial ,2);
				$partial_for_this_float = floatval($partial_for_this_number);
				$amount_total = $key->total_amt - $partial_for_this_float;
				if($key->sign == '+'){
					$invoices .= '
					<tr id="'.$key->ar_id.'">
					<td ><button type="button" onclick="addInvoice(this)" id="invoiceAdd-'.$key->ar_id.'" class="btn btn-success" >Add</button><span> </span><button type="button" class="btn btn-danger"  id="invoiceRemove-'.$key->ar_id.'" onclick="removeInvoice(this)" disabled>Remove</button></td>
					<td id="invoiceRef-'.$key->ar_id.'">'.$key->doc_ref_no.'</td>
					<td id="invoiceDate-'.$key->ar_id.'">'.$key->doc_date.'</td>
					<td id="invoiceAmount-'.$key->ar_id.'">'.$amount_total.'</td>
					</tr>';
				} else {
					$creditNotes .= 
					'<tr id="'.$key->ar_id.'">
					<td ><button type="button" onclick="addCredit(this)" id="creditAdd-'.$key->ar_id.'" class="btn btn-success">Add</button><span> </span><button type="button" class="btn btn-danger" id="creditRemove-'.$key->ar_id.'" onclick="removeCredit(this)" disabled>Remove</button></td>
					<td id="creditRef-'.$key->ar_id.'">'.$key->doc_ref_no.'</td>
					<td id="creditDate-'.$key->ar_id.'">'.$key->doc_date.'</td>
					<td id="creditAmount-'.$key->ar_id.'">'.$amount_total.'</td>
					</tr>';
				}
			}
