...
class Forex_master extends CI_Controller {
	...
	public function delete(){
		...
		//echo $result;
		$name = $this->custom->getSingleValue($this->table,"currency_name",$where);
		$wherecode = array('currency_name' => $name);
		$i = 0;
		$flag = 0;
		do{
			if($i == 0){
				$i++;
				$checkCustomerResult = $this->custom->checkTableValues('customer_master',$where);
