...
defined('BASEPATH') OR exit('No direct script access allowed');
	class Supplier_master extends CI_Controller {
		...
		public function delete()
		{
            ...
            $where = array('supplier_id' => $id); // In payment_master and purchase_master, is used salesman_id
            $i = 0;
            $flag = 0;
            do{
                if($i == 0){
                ...
                elseif($i == 1){
                    $i++;
                    $checkPurchaseMaster = $this->custom->checkTableValues('purchase_master',$where);
                ...
                elseif($i == 2){
                    $where = array('purchase_supplier_id' => $id);
                    $i++;
                    $checkPurchaseMaster = $this->custom->checkTableValues('stock_purchase_master',$where);
                    if($checkPurchaseMaster){
                        echo "errors";
                        $flag = 1;
                    }
            ...
            if($flag == 0){
                $where = array('customer_id' => $id); // In customer_master, is used customer   _id
